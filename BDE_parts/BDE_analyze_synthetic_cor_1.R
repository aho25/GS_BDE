BDE_analyze_cor_1 <- function(DATA, Population) {
  ...
  prod_model <- mixed.solve(DATA$p.probe, Z=DATA$m.probe[,final_features.names])
  ...
  prod_accuracy_final <- as.vector(cor(prod_predicted, DATA$p.probe)) # Control_2 - pheno correlation
  
  ### Correlations_first_generation
  prod_model <- mixed.solve(DATA$p.probe, Z=DATA$m.probe[,features_in_G1.names])
  weight_1_pred <- rep(0, ncol(DATA$m.probe))
  names(weight_1_pred) <- colnames(DATA$m.probe)
  ...
  prod_accuracy_first <- as.vector(cor(prod_predicted, DATA$p.probe)) # Control_2 - pheno correlation
