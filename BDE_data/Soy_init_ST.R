Soy_init_ST <- function() {
  ...
  ### Load data
  p.valid <- read.csv('data_csv/soy_st_pheno_valid.csv')
  m.valid <- read.csv('data_csv/soy_st_markers_valid.csv')
  p.valid <- as.matrix(p.valid[,-1])
  row.names(p.valid) <- m.valid[,1]
  m.valid <- m.valid[,-1]
  row.names(m.valid) <- row.names(p.valid)
  
  p.probe <- read.csv('data_csv/soy_st_pheno_probe.csv')
  m.probe <- read.csv('data_csv/soy_st_markers_probe.csv')
