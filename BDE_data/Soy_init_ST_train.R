...
new.probe<-as.matrix(m.probe)
new.data<-as.matrix(dnorm(x=0, mean = 0, sd = 1, log = FALSE))
new.data

x=seq(-5,5,by=0.1);
y<-as.matrix(pnorm(x,mean=-1,sd=2), nrow=299, ncol=1)
y1<-as.matrix(pnorm(x,mean=-1,sd=2), nrow=299, ncol=1)
y2<-as.matrix(pnorm(x,mean=-1,sd=2), nrow=299, ncol=1)
y3<-rbind(y,y1)
y3<-rbind(y3,y2)
x<-c(301,302, 303)
y4<-y3[-x,]
new.vector<-m.probe*y4
