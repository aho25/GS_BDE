...
Synthetic_Data <- function(OFFSET) {
  ### Load data
  data(soybase)
  dim(gen.qa)
  dim(data.line.qa)
  ...
  podmnogestvo <-
    unique(c(
      which(is.na(Pheno.src$yield)),
      which(is.na(Pheno.src$moisture)),
      which(is.na(Pheno.src$protein)),
      which(is.na(Pheno.src$oil)),
      which(is.na(Pheno.src$fiber)),
      which(is.na(Pheno.src$size))
    ))  #delete 'NA' from traits
  ...
  Init_data <- list(p.valid=p.test, m.valid=m.test, p.probe=p.train, m.probe=m.train)
  return(Init_data)
